...
export const fetchAllTeams = () => dispatch =>
  APIUtil.fetchData(
    "http://data.nba.net/data/10s/prod/v1/2017/teams.json"
  ).then(teams => {
...
export const fetchTeamRoster = urlName => dispatch =>
  APIUtil.fetchData(
    `http://data.nba.net/data/10s/prod/v1/2017/teams/${urlName}/roster.json`
  ).then(teamRoster => dispatch(receiveTeam(JSON.parse(teamRoster))));
